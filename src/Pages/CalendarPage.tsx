...
import CalendarGrid, { type CalendarGridMarker } from '../components/shared/CalendarGrid';
...
const formatShortDate = (value: string) =>
  parseDateValue(value).toLocaleDateString('en-US', { month: 'short', day: 'numeric' });

const formatLongDate = (value: string) =>
  parseDateValue(value).toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric', year: 'numeric' });
  ...
  return value
    .map((item) => {
      ...
      if (item && typeof item === 'object') {
        ...
        const date = typeof record.date === 'string' ? record.date : typeof record.frozenDate === 'string' ? record.frozenDate : '';
        const reason = typeof record.reason === 'string' ? record.reason : typeof record.freezeReason === 'string' ? record.freezeReason : '';
...
const CalendarPage: React.FC = () => {
  ...
  const [scheduleDate, setScheduleDate] = useState(toDateValue(addDays(today, 2)));
  ...
  const [scheduledPanels, setScheduledPanels] = useState<ScheduledPanel[]>(() => loadStoredJson<ScheduledPanel[]>(PANEL_STORAGE_KEY, []));
  const [frozenDates, setFrozenDates] = useState<FrozenDateRecord[]>(() => normalizeFrozenDates(loadStoredJson(FROZEN_STORAGE_KEY, [])));
  ...
  const normalizeList = (data: unknown) => {
    ...
    if (Array.isArray(payload?.supervisors)) return payload.supervisors as Record<string, unknown>[];
    if (Array.isArray(payload?.groups)) return payload.groups as Record<string, unknown>[];
    return [];
  };
  ...
  const openEditPanelDrawer = (panel: ScheduledPanel) => {
    ...
    setSelectedSupervisorIds(
      panel.evaluators
        .map((name) => supervisors.find((supervisor) => supervisor.name === name)?.id)
  ...
  const fetchSupervisors = async () => {
    ...
    try {
      for (const endpoint of endpoints) {
        ...
        const list = normalizeList(data)
          .map((item) => {
            ...
            const name = String(item.name ?? item.full_name ?? item.fullName ?? '').trim();
      ...
      setSupervisorsError(error instanceof Error ? error.message : 'Failed to load supervisors.');
      setSupervisors([]);
    } finally {
  ...
  const fetchGroups = async (level: string) => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/api/groups/level/${level}`, { headers });
      if (!response.ok) {
      ...
      const list = normalizeList(data)
        .map((item) => {
          ...
          const name = String(item.group_name ?? item.groupName ?? item.name ?? '').trim();
          const supervisor = String(item.supervisor_name ?? item.supervisorName ?? item.supervisor ?? 'Not assigned').trim();
          const memberCount = Number(item.member_count ?? item.memberCount ?? (Array.isArray(item.members) ? item.members.length : 0));
  ...
  useEffect(() => {
    window.localStorage.setItem(PANEL_STORAGE_KEY, JSON.stringify(scheduledPanels));
    window.localStorage.setItem(FROZEN_STORAGE_KEY, JSON.stringify(frozenDates));
  ...
  const sortedPanels = useMemo(
    () => [...scheduledPanels].sort((a, b) => `${a.date} ${a.time}`.localeCompare(`${b.date} ${b.time}`)),
    [scheduledPanels],
  );
  ...
  const { cells, markerMap } = useMemo(() => {
    ...
    frozenDates.forEach((value) => {
      ...
      if (frozenDate.getFullYear() !== year || frozenDate.getMonth() !== month) {
        return;
      }
  ...
  const handleScheduleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const selectedEvaluators = supervisors.filter((supervisor) => selectedSupervisorIds.includes(supervisor.id));
    ...
    setScheduledPanels((current) =>
      editingPanelId
        ? current.map((panel) => (panel.id === editingPanelId ? nextPanel : panel))
        : [nextPanel, ...current],
    );
    setDrawerMode('schedule');
    setIsDrawerOpen(false);
  ...
  const deletePanel = (panelId: string) => {
    ...
    setScheduledPanels((current) => current.filter((item) => item.id !== panelId));
  ...
  const handleFreezeSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    setFrozenDates((current) =>
      current.some((item) => item.date === freezeDate)
        ? current.map((item) => (item.date === freezeDate ? { ...item, reason: trimmedReason } : item))
  ...
  const toggleSupervisor = (supervisorId: number) => {
    setSelectedSupervisorIds((current) =>
      current.includes(supervisorId) ? current.filter((value) => value !== supervisorId) : [...current, supervisorId],
    );
  };
  ...
  return (
    <div className="app-layout calendar-shell" style={{ backgroundColor: '#f8fafc', display: 'flex', minHeight: '100vh' }}>
      <Sidebar />
          ...
          <div className="dashboard-content calendar-page-wrap">
            <div className="calendar-header-row">
              <div>
                ...
                <p className="calendar-page-subtitle">Manage evaluation panels, frozen dates, and project schedules</p>
              </div>

              {isCoordinator && (
                <div className="calendar-action-row">
                  <button type="button" className="freeze-date-btn ghost-btn" onClick={() => openDrawer('freeze')}>
                    <CalendarDays size={16} />
                    Freeze Date
                  </button>

                  <button type="button" className="freeze-date-btn" onClick={openCreatePanelDrawer}>
                    <Plus size={16} />
                    Schedule Panel
                  </button>
                </div>
            ...
            <div className="calendar-layout-grid">
              ...
              <aside className="calendar-right-card" aria-label="Upcoming panels">
                <div className="calendar-side-header">
                  <h3>Upcoming Panels</h3>
                  <span className="calendar-side-count">{scheduledPanels.length}</span>
                </div>
                <div className="upcoming-list">
                  {sortedPanels.length === 0 ? (
                    <div className="empty-state-card">
                      <strong>No saved panels yet</strong>
                      <span>Create a panel from the drawer and it will stay after refresh.</span>
                    </div>
                  ) : (
                    sortedPanels.map((panel) => (
                      <article key={panel.id} className="upcoming-item">
                        <div className="upcoming-date-chip">{formatShortDate(panel.date)}</div>
      ...
      {isDrawerOpen && isCoordinator && (
        <div className="drawer-overlay" role="presentation" onClick={closeDrawer}>
          <aside className="schedule-drawer" role="dialog" aria-modal="true" aria-label="Schedule panel drawer" onClick={(event) => event.stopPropagation()}>
            <div className="drawer-header">
              <div>
                <p className="drawer-kicker">Coordinator tools</p>
                <h3>{drawerMode === 'schedule' ? 'Schedule Panel' : 'Freeze Date'}</h3>
              </div>
            ...
            <div className="drawer-switcher">
              <button
                type="button"
                className={drawerMode === 'schedule' ? 'drawer-switch active' : 'drawer-switch'}
              ...
              <button
                type="button"
                className={drawerMode === 'freeze' ? 'drawer-switch active' : 'drawer-switch'}
            ...
            {drawerMode === 'schedule' ? (
              <form className="drawer-form" onSubmit={handleScheduleSubmit}>
                <div className="drawer-summary-card">
                  ...
                  <span>{frozenDates.some((item) => item.date === scheduleDate) ? 'This date is currently frozen.' : 'Date available for scheduling.'}</span>
                </div>
                ...
                <label className="drawer-field">
                  ...
                  <select value={selectedGroupId} onChange={(event) => setSelectedGroupId(event.target.value)} disabled={groupsLoading || groups.length === 0}>
                    <option value="">{groupsLoading ? 'Loading groups...' : 'Choose a group'}</option>
                    {groups.map((group) => (
                      <option key={String(group.id)} value={String(group.id)}>
                        {group.name} {group.supervisor ? `• ${group.supervisor}` : ''}
                      </option>
                    ))}
                  </select>
                ...
                <label className="drawer-field">
                  ...
                  <div className="supervisor-picker">
                    {supervisorsLoading ? (
                      ...
                      <div className="drawer-help error">{supervisorsError}</div>
                    ) : supervisors.length === 0 ? (
                      <div className="drawer-help">No supervisors available.</div>
                    ) : (
                      supervisors.map((supervisor) => (
                        <button
                          key={supervisor.id}
                          type="button"
                          className={selectedSupervisorIds.includes(supervisor.id) ? 'supervisor-chip active' : 'supervisor-chip'}
                  ...
                  {selectedSupervisorIds.length > 0 && (
                    <div className="selected-summary">
                      {selectedSupervisorIds.map((id) => {
                        ...
                        return supervisor ? <span key={id}>{supervisor.name}</span> : null;
                      })}
                    </div>
                  )}
                </label>
                ...
                <label className="drawer-field">
                  ...
                  <input type="url" value={meetingLink} onChange={(event) => setMeetingLink(event.target.value)} placeholder="https://..." />
                </label>

                <label className="drawer-field">
                  <span>Notes</span>
                  <textarea value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="Add context for the panel..." />
                </label>

                <div className="drawer-actions">
                  <button type="button" className="drawer-secondary-btn" onClick={closeDrawer}>Cancel</button>
                  <button type="submit" className="drawer-primary-btn">{editingPanelId ? 'Update Panel' : 'Save Panel'}</button>
                </div>
              </form>
            ) : (
              <form className="drawer-form" onSubmit={handleFreezeSubmit}>
                <div className="drawer-summary-card freeze-card">
                  ...
                  <span>Freezing a date blocks panel scheduling for that day.</span>
                </div>

                <label className="drawer-field">
                  <span>Freeze Date</span>
                  <input type="date" value={freezeDate} onChange={(event) => setFreezeDate(event.target.value)} />
                </label>

                <label className="drawer-field">
                  <span>Reason</span>
                  <textarea value={freezeReason} onChange={(event) => setFreezeReason(event.target.value)} placeholder="Reason for freezing this date" />
                </label>
                ...
                <div className="drawer-field frozen-preview">
                  ...
                  <div className="frozen-list">
                    {frozenDates.map((value) => (
                      <span key={value.date} title={value.reason || 'No reason added'}>
                        {formatShortDate(value.date)}{value.reason ? ` - ${value.reason}` : ''}
                      </span>
                    ))}
                  </div>
                </div>
                ...
                <div className="drawer-actions">
                  <button type="button" className="drawer-secondary-btn" onClick={closeDrawer}>Cancel</button>
                  <button type="submit" className="drawer-primary-btn">Freeze Date</button>
                </div>
              </form>
            )}
          </aside>
