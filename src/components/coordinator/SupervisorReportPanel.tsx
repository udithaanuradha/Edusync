...
const PENDING_GRADE: GradeDefinition = {
  min: 0,
  max: 0,
  letter: 'Pending',
  badgeBg: '#f1f5f9',
...
// Vibrant, bright palette for the distribution bar
const DISTRIBUTION_COLORS: Record<string, string> = {
  'A+': '#059669', // Emerald vibrant
  ...
  'B-': '#60a5fa', // Sky blue
  'C+': '#8b5cf6', // Violet
  C: '#f59e0b',    // Amber / warm gold
  'C-': '#f97316', // Orange
  D: '#fb923c',    // Coral / light orange
  I: '#ef4444',    // Crimson red
};
...
const SupervisorReportPanel: React.FC<SupervisorReportPanelProps> = ({
  ...
  const fetchReportData = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Chronological stage order: Proposal (1) -> Interim (2) -> Code Review (3) -> Final (4)
        const getStageRank = (name: string): number => {
          const n = name.toLowerCase().replace(/[\s_-]+/g, '');
          if (n.includes('proposal')) return 1;
          if (n.includes('interim')) return 2;
          if (n.includes('codereview') || n.includes('review')) return 3;
          if (n.includes('final')) return 4;
          return 5;
        };

        const canonicalList = Array.from(stageGroupMap.values()).sort((a, b) => {
          const rankA = getStageRank(a.stage_name);
          const rankB = getStageRank(b.stage_name);
        ...
        const processed: StudentReportItem[] = evaluatedGroupData.map((item: any) => {
          ...
          // Ethical grading logic: If student has NOT been evaluated yet (stagesEvaluatedCount === 0),
          // their grade is NOT 'I' (Incomplete/Fail); their grade is 'Pending' and final mark is null.
          const isEvaluated = stagesEvaluatedCount > 0 && sumMax > 0;
          let finalScore: number | null = null;
          let gradeInfo: GradeDefinition = PENDING_GRADE;
  ...
  const degreeStats = useMemo(() => {
    ...
    const sumMarks = evaluatedStudents.reduce((acc, s) => acc + (s.final_mark || 0), 0);
    const passCount = evaluatedStudents.filter((s) => isEligibleForPromotion(s.gradeInfo.letter, s.final_mark || 0)).length;
    ...
    Object.keys(gradeCounts).forEach((letter) => {
      gradePercentages[letter] = Number(((gradeCounts[letter] / evaluatedStudents.length) * 100).toFixed(1));
    });
    ...
    return {
      ...
      pendingCount,
      avgMark: Number((sumMarks / evaluatedStudents.length).toFixed(2)),
      passCount,
      passRate: Number(((passCount / evaluatedStudents.length) * 100).toFixed(1)),
      gradeCounts,
      gradePercentages,
    };
  }, [degreeStudents]);
  ...
  const handleDownloadPdfReport = async () => {
    ...
    try {
      ...
      showToast(`Error downloading report: ${err instanceof Error ? err.message : 'Unknown error'}`, 'error');
    } finally {
      ...
      {toast && (
        <div
          style={{
            ...
            gap: '12px',
            padding: '14px 20px',
            backgroundColor: toast.type === 'error' ? '#991b1b' : toast.type === 'warning' ? '#0f172a' : '#166534',
            color: '#ffffff',
            borderRadius: '10px',
            boxShadow: '0 10px 25px -5px rgba(0, 0, 0, 0.25), 0 8px 10px -6px rgba(0, 0, 0, 0.1)',
            fontSize: '13.5px',
            fontWeight: '600',
            border: `1px solid ${toast.type === 'error' ? '#ef4444' : toast.type === 'warning' ? '#f59e0b' : '#22c55e'}`,
          }}
        >
          {toast.type === 'error' && <AlertCircle size={18} color="#fca5a5" style={{ flexShrink: 0 }} />}
          {toast.type === 'warning' && <AlertCircle size={18} color="#f59e0b" style={{ flexShrink: 0 }} />}
          {(!toast.type || toast.type === 'success' || toast.type === 'info') && (
      ...
      >
        ...
        {degreeStats.evaluatedCount > 0 ? (
          ...
          <div style={{ padding: '16px', textAlign: 'center', backgroundColor: '#f8fafc', borderRadius: '8px', color: '#94a3b8', fontSize: '13px' }}>
            No evaluations completed yet. Grade distribution will display once marks are entered.
          </div>
        )}
