...
const Sidebar: React.FC = () => {
  ...
  const toggleMenu = (menuKey: string): void => {
    if (!collapsed) {
  ...
  const menuItems: MenuItem[] = [
    ...
    { path: '/dashboard/approval', icon: ClipboardCheck, label: 'Approval' },
    { path: '/dashboard/schedule', icon: CalendarDays, label: 'Schedule Meeting' },
  ];
          ...
          <div key={item.key || item.path || index}>
            {item.hasSubmenu && item.key ? (
              <>
                ...
                {!collapsed && expandedMenus[item.key] && item.submenu && (
